...
def predict(model, datapath):
    ...
    for weight_path in models:
        ...
        for i in range(len(test_generator.img_dir)):
            text_diction[test_generator.img_dir[test_generator.indexes[i]]] = decoded_res[i]
            print('{}: {}'.format(test_generator.img_dir[test_generator.indexes[i]], decoded_res[i]))
    return text_diction

if __name__=='__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", default='kfold_model/', type=str)
    parser.add_argument('--data', default='Newfolder/', type=str)
